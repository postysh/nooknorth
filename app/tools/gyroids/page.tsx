...
export default function GyroidsPage() {
  const [gyroids, setGyroids] = useState<Gyroid[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");

  useEffect(() => {
    async function fetchGyroids() {
      try {
        const res = await fetch("/api/gyroids");
  ...
  const filtered = useMemo(() => {
    return gyroids.filter((g) => g.name.toLowerCase().includes(search.toLowerCase()));
  }, [gyroids, search]);
    ...
    >
      <div className="flex-1 overflow-y-auto pt-20 px-4 pb-8" style={{ scrollbarGutter: 'stable' }}>
        <div className="max-w-7xl mx-auto">
          <div className="mb-6">
            <h1 className="text-2xl font-semibold text-foreground mb-1">🎵 Gyroids</h1>
            <p className="text-sm text-muted-foreground">
              {loading ? "Loading..." : `${filtered.length} gyroids`}
            </p>
          </div>
              ...
              {filtered.map((g) => (
                ...
                >
                  ...
                  <div className="px-3 pb-3">
                    <h3 className="font-medium text-sm text-foreground truncate">{g.name}</h3>
                    <p className="text-xs text-muted-foreground truncate">{g.sound}</p>
